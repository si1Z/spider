...
def onlySetUrlList(n = 5):
    ...
    if client.exists('xueqiu_base_redis:start_urls'):
        return
    ...
    for i in range(n):
        for code50 in code_list_list:
            ...
            client.rpush('xueqiu_base_redis:start_urls', url)

def onlyEmptyTaskList():
    client = redis.Redis(host='10.11.9.12', port=6379, db=0, password='foobared', decode_responses=True)
    if not client.exists('xueqiu_base_redis:start_urls'):
        return
    for i in range(100):
        client.delete('xueqiu_base_redis:start_urls')
        time.sleep(.5)
